...
interface Sellers {
  ...
  offerElement: IOffers;
}

export const Sellers = ({ data, offerElement }: Sellers) => {
  const { selectedOfferIsInStore, addComponent, removeComponent } =
    useComponentsStore();
  ...
  const isInStore = selectedOfferIsInStore({
    searchTableName: {
  ...
  const handleAddToConfigurationClick = () => {
    if (!isInStore) {
      addComponent({
        searchTableName: searchTableNameNames[data.componentType],
        data: data,
        offer: offerElement,
        countOfComponents: 1,
      });
  ...
  return (
    <div className={styles.sellersContainer}>
      <p className={styles.sellersShop}>{offerElement.store.name}</p>
      <p className={styles.sellersPrice}>
        {offerElement.price.toLocaleString()} ₽
      </p>
      <div className={styles.buttonsContainer}>
        <button
          onClick={() => {
            handleAddToConfigurationClick();
          }}
        ...
        <a
          href={offerElement.buyLink}
          target="_blank"
          className={styles.sellersBuyButton}
        >
          <p>Купить</p>
        </a>
      </div>
