...
export const CatalogComponent = memo(() => {
  ...
  return (
    <div className="min-w-full mx-[15px] h-full overflow-y-auto overscroll-contain grid grid-cols-4 gap-[20px] max-lg:gap-[10px] max-md:gap-[5px] max-xl:grid-cols-3 max-lg:grid-cols-2 max-sm:grid-cols-1 max-lg:min-h-[100%]">
      {catalogItemsArr.map((column, index) => {
        return (
          <div className="flex flex-col self-start " key={index}>
            <button className="text-[20px] font-[800] mb-[10px] text-left">
              {column.title.ru}
            </button>
